...
router.post('/generate', async (req, res) => {
    try {
        ...
        let tempVideoPath;
        // Handle remote vs. local video path
        if (videoUrlToUse.startsWith('http')) {
            tempVideoPath = path.join(__dirname, '..', '..', 'uploads', 'temp', `download_${Date.now()}.mp4`);
            const writer = fs.createWriteStream(tempVideoPath);
            const response = await axios({ url: videoUrlToUse, method: 'GET', responseType: 'stream' });
            ...
            // It's a local path, construct it relative to the project root
            tempVideoPath = path.join(__dirname, '..', '..', videoUrlToUse);
        }
        ...
        const reframedDestPath = path.join(__dirname, '..', '..', 'uploads', 'clips', 'reframed', sanitizedOutputName);
        ...
        const clipBlobName = `clips/reframed/${sanitizedOutputName}`;
        await bucket.upload(reframedDestPath, {
            destination: clipBlobName,
        });

        // Construct the public URL
        const reframedUrl = `https://storage.googleapis.com/${bucket.name}/${clipBlobName}`;

        // Clean up local file after upload
        fs.unlinkSync(reframedDestPath);
...
router.post('/analyze', async (req, res) => {
    try {
        ...
        // Handle remote vs. local video path
        if (videoUrlToUse.startsWith('http')) {
            videoPath = path.join(__dirname, '..', '..', 'uploads', 'temp', `download_${Date.now()}.mp4`);
            ...
            // It's a local path, construct it relative to the project root
            videoPath = path.join(__dirname, '..', '..', videoUrlToUse);
        }
        ...
        // Generate the preview frame locally first
        const localPreviewPath = await generatePreviewFrame(videoPath, cropParams);
        
        // Upload the preview to GCS
        const previewFilename = path.basename(localPreviewPath);
        const gcsPreviewDest = `previews/${previewFilename}`;

        await bucket.upload(localPreviewPath, {
            destination: gcsPreviewDest
        });

        // Construct the public URL
        const previewUrl = `https://storage.googleapis.com/${bucket.name}/${gcsPreviewDest}`;

        // Clean up the local temporary preview file
        ...
        res.json({
            ...
            analysis: {
                ...
                previewUrl: previewUrl // Return the public GCS URL
            }
        });
